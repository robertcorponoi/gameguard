...
export default class Room {
  ...
  set capacity(capacity) {
    if (capacity < this.playerCount) throw new Error('The new capacity is lower than the current amount of players in this room');
   ...
   */
  add(player: Player) {
    if (this.playerCount === this.capacity) return;
  ...
  broadcast(type: string, contents: string) {
    this.players.map((player: Player) => player.message(type, contents));
  }
}